...
//double timeKernelSUNMatScaleAddI;
...
void set_data_gpu(CVodeMem cv_mem)
{
  ...
  int nnz = SM_NNZ_S(J);
  int nrows = SM_NP_S(J);
  int* indexvals = new int[nnz];
  int* indexptrs = new int[nrows+1];
  double *ewt = NV_DATA_S(cv_mem->cv_ewt);

  //N_Vector u;
  //u = N_VNew_Cuda(nrows);  /* Allocate u vector */

  //todo fix int sunindextype
  //printf("%d ", sizeof(int)); //4
  //printf("%d ", sizeof(sunindextype)); //8
  for (int i=0; i<nnz; i++){
    indexvals[i] = SM_INDEXVALS_S(J)[i];
  }

  for(int i=0;i<nrows+1;i++){
    indexptrs[i] = SM_INDEXPTRS_S(J)[i];
  }


  bicg.setUpSolver(ewt,nrows,nnz,
                     (double*)SM_DATA_S(J),indexvals,indexptrs,1);
...
int cvStep_gpu(CVodeMem cv_mem)
{
  ...
  cudaMemcpyDToGpu(ewt, bicg.dewt, bicg.nrows);
  ...
  cvPrepareNextStep_gpu(cv_mem, dsm);
...
int cvNlsNewton_gpu(CVodeMem cv_mem, int nflag)
{
  ...
  double *acor_init = NV_DATA_S(cv_mem->cv_acor_init);
  double *cv_y = NV_DATA_S(cv_mem->cv_y);
  double *tempv = NV_DATA_S(cv_mem->cv_tempv);
  double *ftemp = NV_DATA_S(cv_mem->cv_ftemp);

  double zn[NUM_TESTS*bicg.nrows];
  for(int i=0; i<NUM_TESTS; i++){
    double *aux_zn = NV_DATA_S(cv_mem->cv_zn[i]);
    //memcpy(zn+i*bicg.nrows,aux_zn,sizeof(double)*bicg.nrows);
    for (int j = 0; j < bicg.nrows; j++) {
      zn[i*bicg.nrows+j] = aux_zn[j];
    }
  }

  cudaMemcpyDToGpu(acor, bicg.dacor, bicg.nrows);
  cudaMemcpyDToGpu(zn, bicg.dzn, bicg.nrows*NUM_TESTS);
  cudaMemcpyDToGpu(tempv, bicg.dtempv, bicg.nrows);
  ...
  vtemp1 = cv_mem->cv_acor_init;  /* rename acor as vtemp1 for readability  */
  vtemp2 = cv_mem->cv_acor_init;  /* rename y as vtemp2 for readability     */
  vtemp3 = cv_mem->cv_acor_init;  /* rename tempv as vtemp3 for readability */
  ...
  for(;;) {
    ...
    cudaMemcpyDToCpu(cv_y, bicg.dcv_y, bicg.nrows);
...
//}
//todo check for jac index changes in sunmatscaleaddI
//todo receive jac_data_gpu from camp
//Solution: wait until late (when all cvode functions are in gpu), and aislate gpu
//ODE solving method from CVODE. In this way, CVODE remains aislated for CPU and
//our method doesn't affect future updates from CVODE
...
int linsolsetup_gpu(CVodeMem cv_mem,int convfail,N_Vector vtemp1,N_Vector vtemp2,N_Vector vtemp3)
{
  ...
  if (jok) {
    ...
    retval = SUNMatZero(cvdls_mem->A);//todo we already set this to zero in our calc_jac

    //clock_t start5 = clock();

    //int Jac(realtype t, N_Vector y, N_Vector deriv, SUNMatrix J, void *solver_data,
    //        N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
    retval = cvdls_mem->jac(cv_mem->cv_tn, cv_mem->cv_y,
...
int linsolsolve_gpu(CVodeMem cv_mem)
{
  ...
  for(;;) {
    ...
    // Call the lsolve function //todo improve this part
    cudaMemcpyDToCpu(tempv, bicg.dtempv, bicg.nrows);
    b = tempv;//NV_DATA_S(cv_mem->cv_tempv);

    bicg.solveGPU(b, x); //Linear solving
    cudaMemcpyDToCpu(x, bicg.dx, bicg.nrows);//todo necessary?
    ...
    if (dcon <= 1.0) {
      ...
      cudaMemcpyDToCpu(b, bicg.dx, bicg.nrows);
      cudaMemcpyDToCpu(acor, bicg.dacor, bicg.nrows);
      return (CV_SUCCESS);
    }
    cv_mem->cv_mnewt = ++m;
    ...
    if ((m == cv_mem->cv_maxcor) || ((m >= 2) && (del > RDIV * delp))) {
      ...
      //cudaMemcpyDToCpu(x, bicg.dx, bicg.nrows); //return x
...
void free_gpu(CVodeMem cv_mem)
{
  ...
  //HANDLE_ERROR(cudaFree(cv_mem->indexvals_gpu));
  //HANDLE_ERROR(cudaFree(cv_mem->indexptrs_gpu));
  //HANDLE_ERROR(cudaFree(cv_mem->jac_data_gpu));
...
__global__ void matscaleaddi_global(int nrows, double* dA, sunindextype* djA, sunindextype* diA, double gamma) {
 ...
 int matscaleaddi_gpu(double gamma, SUNMatrix J, CVodeMem cv_mem)
{
